...
<div className= {` ${isVisible ? 'w-[250px]' : 'w-[59px]'} h-screen bg-white justify-start items-start flex`}>
    <div className={` ${isVisible ? 'w-[250px]' : 'w-[59px]'} self-stretch pt-4 flex-col justify-start items-start gap-4 inline-flex`}>
    <div className=" pl-5 pt-5 justify-start items-start gap-2.5 inline-flex">
            ...
            {isVisible &&
            <img className="text-gray-900 text-2xl font-bold leading-9" alt="logo" src={Sapiologie}/>
            }
        </div>
        <div className="w-[59px] h-screen grow shrink basis-0 flex-col justify-between items-start gap-4 flex">
            <div className="self-stretch h-auto flex-col justify-start items-start gap-4 flex">
                {isVisible?  
                <ul className="h-auto pt-4 flex-col justify-start items-start gap-4 flex">
                {Menu1.map((Menu) => (
            ...
            <div className="flex">                    
            <button className={` ${isVisible ? 'w-[250px] pl-5 gap-2.5 justify-start' : 'w-[59px] justify-center py-2'} h-10 items-center gap-[17px] inline-flex`} type="button" onClick={() => handleClick()}>
                        {
                            isVisible ? <img className="min-w-6 min-h-6 relative text-gray-600 " alt="chevron gauche" src={ChevronGauche } />
                            :<img className="min-w-6 min-h-6 relative text-gray-500 " alt="chevron droite" src={ChevronDroite} />

                        }
                    {isVisible &&
                    <p className=" text-gray-600 text-base font-normal leading-normal">Collapse</p>
                    }
                    </button>
                </div>
                <div className={` ${isVisible ? 'w-[250px] pl-5 gap-2.5 justify-start' : 'w-[59px] justify-center py-2'} h-10 items-center gap-[17px] inline-flex`}>
                    ...
                    {isVisible &&
                    <div className=" text-gray-600 text-base font-normal leading-normal">John Doe</div>
                    }
                </div>
            </div>
        </div>
